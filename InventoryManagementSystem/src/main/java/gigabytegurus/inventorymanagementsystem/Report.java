...
public class Report {
	...
	public void generatePDF(List<Item> items, String filePath) {
		try (Document document = new Document()) {
			...
			document.add(new Paragraph(INVENTORY_REPORT));
			document.add(new Paragraph("Generated on: " + new Date()));
			document.add(new Paragraph("\n"));

			// Loop through each item and write its details into the PDF
			for (Item item : items) {
				String text = "ID: " + item.getItemId() + ", Name: " + item.getName() + ", Category: "
						+ item.getCategory() + ", Size: " + item.getSize() + ", Colour: " + item.getColour()
						+ ", Price: €" + item.getPrice() + ", Quantity: " + item.getQuantity() + ", Supplier: "
	...
	public void generateExcel(List<Item> items, String filePath) {
		...
		try (Workbook workbook = new XSSFWorkbook()) {
			...
			Sheet sheet = workbook.createSheet(INVENTORY_REPORT);

			// Title row
			Row titleRow = sheet.createRow(0);
			titleRow.createCell(0).setCellValue(INVENTORY_REPORT);
			...
			for (Item item : items) {
				...
				Cell[] cells = new Cell[] { row.createCell(0), row.createCell(1), row.createCell(2), row.createCell(3),
						row.createCell(4), row.createCell(5), row.createCell(6), row.createCell(7) };
